...
public class MenuController {
    ...
    public ApiResult getById(IdDto dto) {
        ...
        return ApiResult.success("查询成功", iMenuService.getById(dto.getId()));
    }
    ...
    @PostMapping("/insert")
    @ApiOperation("添加系统菜单")
    public ApiResult insert(@RequestBody MenuInsertDto dto) {
        ...
        String warn = iMenuService.insert(dto);
    ...
    @PutMapping("/update")
    @ApiOperation("修改系统菜单")
    public ApiResult update(@RequestBody MenuUpdateDto dto) {
        if (ObjectUtils.isEmpty(dto.getId())) {
            return ApiResult.warn("id不能为空");
        ...
        String warn = iMenuService.update(dto);
        if (!ObjectUtils.isEmpty(warn)) {
            return ApiResult.warn(warn);
        }
